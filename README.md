...
| Регион | RPS | Средний трафик, Гбит/с | Пиковый трафик, Гбит/с |
|:-:|-|-|-|
| Северная Америка | 1393.249 | 12.254 | 24.507 |
| Европа | 1555.13 | 13.677 | 27.355 |
| Азия | 1375.821 | 12.1 | 24.201 |
...
| File | 500.527 Кб | 1.124 * 10^9 | 523.787 Тб |
...
Question | user_id | master-slave
Answer | по question_id | master-slave
Post | user_id | master-slave
...
UserActivity | по user_id | master-slave

Для организации геораспределенной БД будем использовать отдельный кластер на каждый ДЦ с асинхронной репликацией между другими ДЦ с помощью очереди. Шардирование между ДЦ будем осуществлять по user_id. Таким образом данные от пользователей будут записываться в ближайший ДЦ и считываться с локальной реплики.

### Клиентские библиотеки / интеграции

* PostgreSQL - pgx
* Citus - Расширение PostgreSQL для создания распределенной БД
* Kafka - Очередь для асинхронной репликации данных
...
В ленту добавляем посты, темы которых совпадают с интересами пользователя, случайные посты и популярные в данный момент. Удаляем совпадения и добавляем рекламные посты.
